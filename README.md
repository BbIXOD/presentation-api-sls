...
- Також, особливо через секцію функцій, можна виконати цей крок вже після наповнення. Тому думайте.
...
### Створення, власне, REST-у
Ну дивіться, в чому суть цього поняття? Як ми вже побачили з презентації нашої однокурсниці, суть (як мінімум для нас як програмістів) у використанні json і методів по типу get, post і так далі. Код у мене є в проєкті, але давайте зробимо набросок що за що відповідає:
- `GET` - Має повертати інформацію про всі або конкретну сутність таблиці. Тобто ми робимо `SELECT` запит з можливою умовою, що, наприклад, видає лише один обьєкт з потрібним айді
- `POST` - Створення об'єкта. Можна зробити `SELECT` перевірку на існування, але найголовніше — `INSERT` нового елементу в таблицю.
- `PUT` - Створення\зміна. для мене цей обьєкт має забагато повноважень тому я використовую наступний метод.
- `PATCH` - Схожий метод. суть передає `ALTER`. Теж буде не зайвою перевірка на наявність. 
- `DELETE` - Ну тут все примітивно. Потрібно видалити елемент і навіть SQL містить те ж слово. Але перевірка на наявність теж може стати в нагоді.
Також правильно розділяйте path i body параметри для покращення безпеки в додатку.

Але як же прив'язати саме потрібний тип запиту? Все елементарно:
```
events:
      - http:
          path: /category
          method: *СЮДИ ПИШІТЬ МЕТОД*
```
Ось так ми це прикріплюємо.
### Примітки
...
![Тестування в браузері](img/testing-browser.png)
![Тестування в postman](img/testing-common.png)
